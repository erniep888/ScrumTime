<%@ Control Language="C#" Inherits="System.Web.Mvc.ViewUserControl<ScrumTime.Models.Task>" %>
    <tr id="taskRow_<%: Model.TaskId %>" class="taskRow">
        ...
        <td style="vertical-align:middle;text-align:center;width:194px;border:0px;font-size:11px;"><a href="#" class="simpleLink" onclick="deleteTask(<%: Model.StoryId %>, <%: Model.TaskId %>, '<%: Url.Action(MVC.Task.Delete()) %>', '<%: Url.Action(MVC.Task.TotalTaskHours()) %>');">Delete</a> | <a class="simpleLink" href="#">Move</a></td>
        <script type="text/javascript">           
            setupReadOnlyTaskRow(<%: Model.TaskId %>, <%: Model.StoryId %>, '<%: Url.Action(MVC.Task.Edit()) %>');
        </script>
    </tr>