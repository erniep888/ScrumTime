...
class SharedTagLib {
    ...
    def header = {attrs ->
        out << render(template: "/shared/header")
    }

    def footer = {attrs ->
        out << render(template: "/shared/footer")
    }

    def mainmenu = {attrs ->
        out << render(template: "/shared/mainMenu")
    }

    def messagepane = {attrs ->
        out << render(template: "/shared/appMessagePane",
                      model:[messageCode:attrs.messageCode])
    }

    def editfield = {attrs ->
        out << render(template: "/shared/formEditField",
    ...
    def textareafield = {attrs ->
        out << render(template: "/shared/formTextAreaField",
                      model:[useSemiColon:attrs.useSemiColon,
                             labelClassValue:attrs.labelClass,
                             fieldClassValue:attrs.fieldClass,
                             fieldName:attrs.fieldName,
