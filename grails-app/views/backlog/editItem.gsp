...
<div style="margin-top:8px;padding-bottom:16px;width:100%;text-align:left;border:#999 solid 1px;">
    <div style="text-align:left;border-bottom:#999 solid 1px;background-color:#c6d5ec;">
        <span style="padding-left:6px">Edit Item - <span style="color:blue">Product Backlog</span></span>
    </div>
    <g:hasErrors bean="${backlogItem}">
        <div class="errors">
            <g:renderErrors bean="${backlogItem}" field="title" as="list"/>
            <g:renderErrors bean="${backlogItem}" field="description" as="list"/>
            <g:renderErrors bean="${backlogItem}" field="workEstimate" as="list"/>
            <g:renderErrors bean="${backlogItem}" field="estimatedBy" as="list"/>
        </div>
    </g:hasErrors>
        ...
        <g:form controller="backlog">
            ...
            <tr>
                <td colspan="2"><input name='title' maxlength="256"
                    ...
                    value="${backlogItem?.title?.encodeAsHTML()}"/></td>
            </tr>
            <tr>
            ...
            <tr>
                <td colspan="2"><textarea  name='description' cols="30" rows="5"
                    style="width:500px;height:50px;font-family:arial;font-size:13px">${backlogItem?.description?.encodeAsHTML()}</textarea></td>
            </tr>
            <tr>
                <td colspan="2" style="vertical-align:top;width:100px;font-size:12px;padding-top:8px">Work Estimate (hours)</td>
            </tr>
            <tr>
                <td colspan="2"><input style="font-size:13px;width:30px;text-align:center;" type="text" name="workEstimate" value="${backlogItem?.workEstimate}" maxlength="4" size="4"/></td>
            </tr>
            <tr>
                <td colspan="2" style="vertical-align:top;width:100px;font-size:12px;padding-top:8px">Estimate By</td>
            </tr>
            <tr>
                <td colspan="2"><g:select style="font-size:12px" name="estimatedBy" value="${backlogItem?.estimatedBy?.encodeAsHTML()}"
                            from="${users}" optionKey="nickName" optionValue="nickName"
                            noSelection="['-1':'----------------------']"/></td>
            </tr>
            <tr>
                <td style="text-align:right;padding-top:6px;padding-right:16px;width:425px"><g:hiddenField name="savePressed" value="true"/><g:hiddenField name="id" value="${backlogItem?.id}"/><g:actionSubmit value="Save" action="edit" style="width:60px"/></td>
